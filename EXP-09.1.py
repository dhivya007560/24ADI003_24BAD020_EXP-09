...
movies_plot = [str(i[0]) for i in recommendations]
scores = [i[1] for i in recommendations]

plt.figure(figsize=(8,5))
plt.bar(movies_plot, scores)
plt.xlabel("Movie ID")
...
sns.heatmap(user_item.iloc[:20, :20], annot=False)
plt.title("User-Item Matrix Heatmap")
plt.xlabel("Items (Movies)")
plt.ylabel("Users")
plt.show()
plt.figure(figsize=(10,8))
sns.heatmap(user_similarity_df.iloc[:20, :20], annot=False)
